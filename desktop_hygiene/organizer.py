...
def resolve_folder_answer(answer: str, folders: list[str]) -> str:
    """Accept either a menu number or a configured folder name."""

    if answer.isdigit():
        selected_index = int(answer)
        if selected_index < 1 or selected_index > len(folders):
            raise ValueError("Escolha de pasta fora da lista.")
        return folders[selected_index - 1]

    normalized_answer = comparison_key(answer)
    for folder in folders:
        if normalized_answer == comparison_key(folder):
            return folder

    available = ", ".join(folders)
    raise ValueError(f"Escolha de pasta invalida. Use um numero ou um destes nomes: {available}")
...
def prompt_folder_choice(config: dict[str, Any]) -> tuple[dict[str, Any], str | None]:
    ...
    print(f"\nAguardando escolha. Pressione Enter para usar {default_name}.")
    print(f"Dica: para rodar direto, use --folder {default_name}.")
    try:
        answer = input(f"Escolha [1-{len(folders)}], nome da pasta, ou Enter: ").strip()
    except EOFError:
        answer = ""
    if not answer:
        selected = folders[default_index - 1]
    else:
        selected = resolve_folder_answer(answer, folders)
